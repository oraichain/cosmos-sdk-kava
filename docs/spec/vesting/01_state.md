...
1. Verify `BC >= D > 0`
2. Compute `X := min(max(V - DV, 0), D)` (portion of `D` that is vesting, prioritizing vesting coins)
...
To initialize both vesting and non-vesting accounts, the `GenesisAccount` includes the
following fields: `Vesting`, `StartTime`, and `EndTime`. Accounts meant to be
of type `BaseAccount` or any non-vesting type will have `OriginalVesting = 0`. The
genesis initialization logic parses the account fields and returns the correct accounts
accordinglyz.
