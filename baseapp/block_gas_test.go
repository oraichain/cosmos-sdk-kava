...
func TestBaseApp_BlockGas(t *testing.T) {
	testcases := []struct {
		...
		{"exact block gas meter", blockMaxGas, false, true},
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expErr {
				if tc.panicTx {
					require.Equal(t, sdkerrors.ErrPanic.ABCICode(), rsp.Code, "expected panic")
				} else {
					require.Equal(t, sdkerrors.ErrOutOfGas.ABCICode(), rsp.Code, fmt.Sprintf("expected out of gas error err code %d, got err code %d", sdkerrors.ErrOutOfGas.ABCICode(), rsp.Code))
				}
				require.Empty(t, okValue, "expected state to not be modified")
			} else {
				require.Equal(t, uint32(0), rsp.Code, "expected tx to be successful")
				require.Equal(t, []byte("ok"), okValue, "expected state to be modified")
			}
			// check block gas is always consumed
			baseGas := uint64(55419) // baseGas is the gas consumed before tx msg
			expGasConsumed := addUint64Saturating(tc.gasToConsume, baseGas)
			if expGasConsumed > txtypes.MaxGasWanted {
				// capped by gasLimit
				expGasConsumed = txtypes.MaxGasWanted
			}
			require.Equal(t, expGasConsumed, ctx.BlockGasMeter().GasConsumed(), fmt.Sprintf("expected %d to equal %d", ctx.BlockGasMeter().GasConsumed(), expGasConsumed))
			// tx fee is always deducted
			require.Equal(t, int64(0), app.BankKeeper.GetBalance(ctx, addr1, feeCoin.Denom).Amount.Int64(), "expected fee to be paid")
			// sender's sequence is always increased
			seq, err = app.AccountKeeper.GetSequence(ctx, addr1)
			require.NoError(t, err)
			require.Equal(t, uint64(1), seq, "expected sender sequence to be incremented")
		})
	}
}
